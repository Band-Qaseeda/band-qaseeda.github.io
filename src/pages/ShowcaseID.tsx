 ...
 return (
  <>
   ...
   <div className="w-full max-w-7xl mx-auto px-4 lg:px-8 pt-2">
    ...
    {/* Press Release */}
    <h1 className="mt-8 lg:mt-24 text-xl sm:text-2xl md:text-3xl  font-semibold drop-shadow mb-4 border-b-[4px] pb-1 border-accent/50 w-fit">
     Press Release
    </h1>
        ...
        e.link && (
         ...
         >
           ...
           <img
            src={
             "logo" in e && typeof e.logo === "string"
              ? e.logo
              : `https://logo.clearbit.com/${new URL(e.link).hostname}`
            }
            alt="logo"
          ...
          <div className="text-center md:text-left">
           ...
           <p className="text-sm md:text-md mt-2 text-gray-500">{e?.date}</p>
           <a
            href={e?.link}
            target="_blank"
            rel="noreferrer"
           ...
           >
            <button className="bg-accent text-primary rounded-full py-1 px-4 hover:bg-accent/80 transition-all duration-100 flex items-center gap-1 text-sm">
             <span>
              {"logo" in e && typeof e.logo === "string" ? "Watch" : "Read"}
             </span>
             <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6"
